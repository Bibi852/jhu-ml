...
When C~p~, an estimate of test error that penalizes larger $p$, is used to compare the predictive power of models, both forward and backwards selection identifies age, disability, work hours, time to work, gender, and education level as the optimal subset of features for predicting annual income.
...
##### [Ridge (L2) Regression](https://blmoistawinde.github.io/ml_equations_latex/#l2-regularization)
$$\text{Loss} = \text{Error}(Y - \widehat{Y}) +  \lambda \sum_1^n w_i^{2}$$

##### [Lasso (L1) Regression](https://blmoistawinde.github.io/ml_equations_latex/#l1-regularization)
...
def plot_nl_fit(df, basis, title, x, xlab, figsiz = 3):
    ...
    fig, ax = subplots(figsize=(figsiz,figsiz))
...
def bs_plot(d):
    ...
    plot_nl_fit(hw_grid, bs_work, f"Basis Spline, df={d}",
                clean_acs['hrs_work'], 'Hours Worked Per Week', figsiz = 5)
...
# comparing the performance of spline function with different degrees of freedom - for more complexity or smoothness
    ...
    plot_nl_fit(hw_grid, ns_work, f"Natural Spline, df={i}", clean_acs['hrs_work'], 'Hours worked per week', figsiz = 5)
...
```{python}
#| label: smooth-spline
#| fig-cap: Plotting optimal lambda parameter
#| column: margin
...
for lam in np.logspace(-2, 6, 5):
    ...
    ax.plot(hw_grid,
            ...
            label='{:.1e}'.format(lam),
            linewidth=3)
ax.set_xlabel('Hours worked per week', fontsize=8)
...
#| column: margin
# plotting optimal smoothing parameter found through pygam
...
The validated mean-squared error for the generalized additive model of income by male gender and the spline function of hours worked using the default smoothing parameter is 3.03 million, which represents an improvement over the original validation error.  
...
# References
[https://blmoistawinde.github.io/ml_equations_latex/#regression-1](https://blmoistawinde.github.io/ml_equations_latex/#regression-1)
[http://www.deepnlp.org/equation/mean-squared-error-mse](http://www.deepnlp.org/equation/mean-squared-error-mse)
[https://www.youtube.com/watch?v=Xm2C_gTAl8c](https://www.youtube.com/watch?v=Xm2C_gTAl8c)
[https://www.youtube.com/watch?v=NGf0voTMlcs](https://www.youtube.com/watch?v=NGf0voTMlcs)
[https://www.youtube.com/watch?v=Q81RR3yKn30&t=6s](https://www.youtube.com/watch?v=Q81RR3yKn30&t=6s)