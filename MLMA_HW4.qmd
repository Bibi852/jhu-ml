...
#| fig-cap: Annual Income by Categorical Variables
# plotting annual income values by each categorical variable using boxplots
def boxplot_fun(df):
    # get names of categorical attributes
    cat_names = df.select_dtypes(include=['object']).columns
    for i in cat_names:
        plt.figure(figsize = (8,8))
        # plot boxplot
        sns.boxplot(x=df[i],y=acs['income'])
        plt.title(f'Boxplot of Income by {i.capitalize()}')
        plt.xlabel(i.capitalize())
        plt.ylabel('Annual Income')
        plt.show()

boxplot_fun(acs.drop('income', axis = 1))
...
For the first, simplest OLS regression, I want to explore the relationship between gender, hours worked, and income. 
...
```{python}
#| label: initial-ols
# fitting predictors of male gender and hours of work per week
...
# coefficients
ols_coef_summary = summarize(ols_fit)
...
ols_sum_w, ols_sum_l = acc_pred_table(ols_fit, "OLS regression", X, y)
```

OLS regression is a well-established form of supervised learning that models the relationship between a dependent variable and one or more independent variables. OLS regression takes the form of a linear equation, where the y-intercept represents the baseline outcome and a slope coefficient is the change in the outcome associated with a one-unit increase in a given dependent variable when all other predictors are held constant. OLS achieves the best fit to the underlying relationship between features by minimizing the sum of squared errors (the distance between observed and fitted values) with optimal values for the intercept and slope coefficients.  
```{python}
#| label: coef-summary-table
#| echo: false
print(markdown_format(ols_coef_summary, "OLS Coefficients", include_ind = True))
```

From the results of this first OLS model, there seems to be a significant relationship between gender, hours worked per week, and annual income. Specifically, the y-intercept tells us that for women working 0 hours per week, the average annual earnings are $-19,200. This is a negative value and thus not practically interpretable, however the coefficients for `male` and `hrs_work` tell us about the variation in annual income that can be explained by each feature individually. Between two people who work the same amount each week, the man will make $17,900 more on average compared to the woman. A person who works one more hour per week will make $1395.4 more annually than another person of the same gender. Together, gender and hours worked per week explain approximately 14% of the variance in annual income.  

```{python}
#| label: pred-summary-table
#| echo: false
markdown_format(ols_sum_l, 'OLS Predictive Accuracy Summary')
```

The average squared difference between the model's predictions of income and the actual values of income in the `acs12` dataset is $2.9 million. In other words, the model's predictions of annual income deviate from the actual values by approximately $54,000 on average. However, this may not be a honest representation of the model's predictive accuracy as the model was developed specifically to optimize results for the data it was fitted on. 

The coefficients and weights have been altered to reflect any trends or variation in the training dataset, including those attributed to random noise not actually relevant to the underlying relationship between the outcome and predictors. To obtain a more accurate picture of this model's predictive performance, we would thus need to apply it to new, unseen data, and compare the differences between the model's fitted values and the observed ones.  
...
```{python}
# importing libraries from Chapter 7
